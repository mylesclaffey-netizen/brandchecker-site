...
for fam, label in FAMILIES.items():
    ...
    page = swap(page, '<div class="sotu-actions" style="margin-top:8px"><button class="btn2" id="again" type="button">← New comparison</button></div>',
                ...
                + ''.join(' <a class="btn2" href="/make/%s/">%s</a>' % (f, l) for f, l in FAMILIES.items() if f != fam) + ' <a class="btn2" href="/make/stateoftheunion/tracker/">Weekly tracker</a></div>')
...
t = swap(t, '<div class="sotu-actions"><a class="btn2 solid" id="newBtn" href="/tools/state-of-the-union/">New tracker</a></div>',
         '<div class="sotu-actions"><a class="btn2" href="/make/">← Brand Checker for Make</a>' + ''.join(' <a class="btn2" href="/make/%s/">%s</a>' % (f, l) for f, l in FAMILIES.items()) + '</div>')
